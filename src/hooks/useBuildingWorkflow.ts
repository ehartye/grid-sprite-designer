 ...
 * Mirrors useGridWorkflow but parameterized with GridConfig for variable grid sizes.
 */
...
export function useBuildingWorkflow() {
  ...
  }, [state.building, state.model, state.imageSize, state.aspectRatio, dispatch]);
  ...
  const reExtract = useCallback(async (overrides?: {
    ...
    dispatch({ type: 'EXTRACTION_COMPLETE', sprites });
  }, [state.filledGridImage, state.filledGridMimeType, state.imageSize, state.building, dispatch]);
